# SMTP Email Troubleshooting Guide for Docker

## Problem: Emails work locally but not in Docker

### Quick Diagnosis

1. **Check if SMTP credentials are loaded in container:**
```bash
...
2. **Check SMTP configuration via API:**

After logging in to the web interface, visit:
```
http://YOUR_SERVER_IP:5858/api/smtp-config
```
...
```bash
docker-compose logs web | grep -i "smtp\|email\|failed to send"
```
...
### Issue 3: Network connectivity from Docker
